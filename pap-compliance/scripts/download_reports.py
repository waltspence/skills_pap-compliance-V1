...
def main():
    ...
    # ── CO document fetch pass ────────────────────────────────────────
    # CO compliance reports are PRE-GENERATED by backend batch jobs. The client
    # lists existing documents and fetches PDFs — no on-demand generation.
    # See references/co_reports_api.md for the full story.
    ...
    if co_data:
        ...
        if co_queue:
            print(f"\nCO document fetch: {len(co_queue)} patient(s) verified on CO")
        for j, p in enumerate(co_queue):
            result = download_co_reports(co_session, co_headers,
                                         p["patient_uuid"], p["name"],
            ...
            if status == "OK":
                ...
                total = result.get("total_documents", "?")
                print(f"  [{j+1}/{len(co_queue)}] {p['name']} — CO: {dl} PDF(s) "
                      f"downloaded (of {total} total docs)", flush=True)
            elif status == "NO_REPORTS":
                co_failed += 1
                total = result.get("total_documents", 0)
                print(f"  [{j+1}/{len(co_queue)}] {p['name']} — CO: no matching reports "
                      f"({total} total docs on file)", flush=True)
            else:
                co_failed += 1
                print(f"  [{j+1}/{len(co_queue)}] {p['name']} — CO: {status} — "
                      f"{result.get('error', '')}", flush=True)

        with open(log_path, "w") as f:
            json.dump(existing_log + log + co_log, f, indent=2, default=str)
    else:
        print("\nCO session not loaded — skipping CO document fetch.", flush=True)
    ...
    if co_data:
        print(f"  CO — patients with reports downloaded: {co_downloaded}")
        print(f"  CO — patients with no matching docs:  {co_failed}")
